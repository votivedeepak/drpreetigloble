                    ...
                    <div class="card">
                        ...
                        <div class="body">
                            <form id="form_validation" action="{{ url('admin/univer_save') }}" enctype="multipart/form-data" method="POST">
                              @csrf
                             <input type="hidden" name="s_idd" id="s_idd" value="{{isset($uncdata->unc_id)?$uncdata->unc_id:''}}">   
                          
                                <div class="col-sm-6">
                                <div class="form-group form-float">
                                    <div class="form-line">                                      
                                        <input type="text" class="form-control" name="unc_name" value="{{isset($uncdata->unc_name)?$uncdata->unc_name:''}}">
                                     
                                        <label class="form-label">University Category</label>
                                         @if($errors->has("unc_name"))
                                        <span class="invalid-feedback d-block" role="alert" style="color: red;">
                                        <strong>{{$errors->first('unc_name')}}</strong>
                                        </span>
                                        @endif
                                    </div>
                                </div>
                              ...
                              <div class="col-sm-6">
                                <div class="form-group form-float">
                                    <div class="form-line">                                      
                                        <input type="text" class="form-control" name="us_name" value="{{isset($uncdata->us_name)?$uncdata->us_name:''}}">
                                     
                                        <label class="form-label">University Short Name</label>
                                         @if($errors->has("us_name"))
                                        <span class="invalid-feedback d-block" role="alert" style="color: red;">
                                        <strong>{{$errors->first('us_name')}}</strong>
                                        </span>
                                        @endif
                                    </div>
                                </div>
